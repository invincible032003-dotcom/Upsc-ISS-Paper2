...
var OP_NAMES = {log:1, ln:1, exp:1, max:1, min:1, lim:1, sup:1, inf:1, det:1, gcd:1, arg:1, tan:1, sin:1, cos:1, argmax:1, argmin:1, Var:1, Cov:1, Bias:1, MSE:1, Corr:1};
...
MathParser.prototype.readArg = function(){
  ...
  /* A bare (un-braced) argument is exactly ONE character in real LaTeX
     ...
     and leave the remainder in place for continued parsing. */
  if(t && t.t==="TEXT" && t.v.length>1){
  ...
  switch(node.type){
    ...
    case "decorate": {
      ...
      switch(node.deco){
        ...
        case "bb": {
          if(node.body && node.body.type==="text" && node.body.v.length===1 && MATHBB_MAP[node.body.v]){
            return escapeHtmlPlain(MATHBB_MAP[node.body.v]);
          }
          return '<b>'+body+'</b>';
        }
        default: return body;
